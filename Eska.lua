...
function library:New(name) 
	...
	cursor.ZIndex = 100
	cursor.ImageColor3 = COL3RGB(30, 115, 255) 
...
for _,lua in pairs(listfiles("lunarlua")) do 
	local luaname = GSUB(lua, "lunarlua\\", "") 
	INSERT(allluas, luaname) 
end 

RunService.RenderStepped:Wait() 

local gui = library:New("إحساس القم") 
...
luascripts:Element("Button", "load", {}, function() 
	loadstring(readfile("lunarlua\\"..values.luas["lua scripts"].lua.Scroll))() 
end) 
luascripts:Element("Button", "Watermark", nil, function()
        ...
        Frame.BorderColor3 = Color3.fromRGB(30, 60, 150)
...
aimbot:Element("Toggle", "Predict enemy location")
aimbot:Element("Slider", "prediction hitchance", {min = 0, max = 200, default = 50})  
...
world:Element("Dropdown", "hitsound", {options = {"none", "skeet", "neverlose", "rust", "bag", "baimware", "osu", "Tf2", "Tf2 pan", "Boom", "Slap", "1", "Minecraft", "jojo", "vibe", "supersmash", "epic", "retro", "quek"}}) 
...
grenades:Element("Button", "crash server", {}, function() 
	...
	RunService.RenderStepped:Connect(function()
		for count = 1, 10 do
			game.ReplicatedStorage.Events.DropMag:FireServer(Plr.Character.Gun.Mag)
		end 
	end) 
end)
...
mt.__namecall = function(self, ...) 
	...
	if method == "FireServer" and self.Name == "HitPart" then 
		...
		if (values.rage.aimbot["Predict enemy location"].Toggle and RageTarget ~= nil) then 
			coroutine.wrap(function() 
